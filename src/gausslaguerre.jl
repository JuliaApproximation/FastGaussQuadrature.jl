...
function gl_bulk(t, d)
    ...
    x5 = d² * d * _t / 720 * evalpoly(tinv, (224, -48*7, -16, -576, 2814, -3815, 1600))
    x7 = -d⁴ * d / 181440 * _t² * evalpoly(tinv, (-285696, -23040*-31, 384*-1346, -320*43, 80*-221, -1727136, 16131880, -48469876, 66424575, -43122800, 10797500))
    x9 = d⁶ * d / 10886400 * _t³ * evalpoly(tinv, (210677760, -5806080*127, 768*1193053, -768*555239, 16128*-641, -1792*-1598, 3360*-7823, -192*15948182, 672*64957561, -212307298152, 518401904799, -714465642135, 566519158800, -241928673000, 43222750000))

    w3 = d² / 6 * (2*t + 3) / (t-1)^3
    w5 = d⁴ / 720 * _t² * evalpoly(tinv, (0, 0, 16*7, 32, 1712, -12408, 27517, -24860, 8000))
    w7 = -d⁶ * _t³ / 90720 * evalpoly(tinv, (0, 0, 2304*-31, -384*-416, 480*43, -320*-89, 80*53752, -50986344, 201908326, -386872990, 393326325, -204917300, 43190000))
    return (x3, x5, x7, x9), (w3, w5, w7)
end
