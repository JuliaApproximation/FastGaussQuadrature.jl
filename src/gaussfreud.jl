...
function asyBesselgen(np, z, alpha, T::Integer, qm, m::Integer, UQ, npb, useQ::Bool)
    ...
    if useQ
        ...
        Rko = (1+0im)zeros(ELT, T-1,2)
        sL = (1+0im)zeros(ELT, 2,2,T-1)
        for m = 1:T-1
...
function getV(alpha, qm, m::Integer, maxOrder::Integer, r)
    ...
    ns = 0:mo
    f = NaN*zeros(ELT, mo+1) # Coefficients in the expansion of \bar{phi}_n(z) or \xi_n(z)
    g = NaN*zeros(ELT, maxOrder-1,mo+1)
    ...
    if (r == 1) # Right disk: near z=1
        f = NaN*zeros(ELT, mo+2)
        ns = [ns; ns[mo+1]+1] # Extend by one because f(1) = 0 while not for left
        u = zeros(ELT, ns[mo+2]+1,ns[mo+2]+2)
        v = zeros(ELT, ns[mo+2]+1,ns[mo+2]+2)
        u[1,1] = 1.0
        v[1,1] = 1.0
        for n = [ns; ns[mo+2]+1]
        ...
        q = zeros(ELT, ns[mo+2]+1)
        rr = zeros(ELT, ns[mo+2]+1) # Coeffs in the expansion of sqrt(2-2*sqrt(1-w))
        for kt = ns
            for l = 0:kt
    ...
    rho = (1+1im)*zeros(ELT, 2*mo+3,mo+1)
    for n = ns
    ...
    OmOdd = (1+1im)*zeros(ELT, mo+1); OmEven = (1+1im)*zeros(ELT, mo+1)
    XiOdd = (1+1im)*zeros(ELT, mo+1); XiEven = (1+1im)*zeros(ELT, mo+1)
    ThOdd = (1+1im)*zeros(ELT, mo+1); ThEven = (1+1im)*zeros(ELT, mo+1)
    OmO = (1+1im)*zeros(ELT, mo+1); OmE = (1+1im)*zeros(ELT, mo+1)
    XiO = (1+1im)*zeros(ELT, mo+1); XiE = (1+1im)*zeros(ELT, mo+1)
    ThO = (1+1im)*zeros(ELT, mo+1); ThE = (1+1im)*zeros(ELT, mo+1)
    for n = ns
        js = 0:n
        for j = js
    ...
    Ts = (1+1im)*zeros(ELT, 2,2,mo+1) # = G_{k,n}^{odd/even} depending on k, overwritten on each new k
    WV = (1+1im)*zeros(ELT, 2,2,maxOrder-1,mo+1)
    for k = 1:(maxOrder-1)
        Ts[:,:,:] = 0
        if r == 1
            if mod(k,2) == 1
...
function getUQ(alpha, qm, m::Integer, maxOrder::Integer)
    ...
    UQ = (1+1im)*zeros(ELT, 2,2,maxOrder-1,ceil(Int,3*maxOrder/2)+2, 4)
    for kt = 0:(maxOrder-2)
